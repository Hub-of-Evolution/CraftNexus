...
#[contracttype]
#[derive(Clone)]
enum ZeroDecimalStakeTokenKey {
    Balance(Address),
}

#[contract]
struct ZeroDecimalStakeToken;

#[contractimpl]
impl ZeroDecimalStakeToken {
    pub fn decimals(_env: Env) -> u32 {
        0
    }

    pub fn mint(env: Env, to: Address, amount: i128) {
        let key = ZeroDecimalStakeTokenKey::Balance(to);
        let balance: i128 = env.storage().instance().get(&key).unwrap_or(0);
        env.storage().instance().set(&key, &(balance + amount));
    }

    pub fn transfer(env: Env, from: Address, to: Address, amount: i128) {
        let from_key = ZeroDecimalStakeTokenKey::Balance(from);
        let to_key = ZeroDecimalStakeTokenKey::Balance(to);
...
fn test_zero_decimal_stake_below_canonical_minimum_is_rejected() {
    ...
    let token = env.register_contract(None, ZeroDecimalStakeToken);
    let token_client = ZeroDecimalStakeTokenClient::new(&env, &token);
    token_client.mint(&buyer, &100);
    token_client.mint(&seller, &1);

    // One 0-decimal token is 10_000_000 canonical units.
    client.set_min_stake_required(&10_000_000);
    client.stake_tokens(&seller, &token, &1);
    client.set_min_stake_required(&20_000_000);
    client.create_escrow(&buyer, &seller, &token, &1, &1, &Some(3_600));
}
