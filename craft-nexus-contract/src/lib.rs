...
/// # Determinism (#1137)
///
/// Every field is derived from a deterministic projection of live contract
/// storage:
/// - **versioned records**: `contract_version`, `storage_layout_version`,
///   `upgrade_history_len` (versioned upgrade/record counts).
/// - **balances**: cumulative scalar aggregates (`total_locked` from the
///   monotonic `TotalVolume` accumulator). No per-token enumeration is used,
///   so summation order can never affect the result.
/// - **permissions**: whitelisted-token count, upgrade-signer count, pending
///   admin actions, and the stored admin recovery configuration.
/// - **pending jobs**: recurring escrow count + next id, pending batch escrow
///   jobs, and whether a WASM upgrade proposal is currently open.
///
/// ## Sensitive-data handling
///
/// The snapshot deliberately commits to **counts, sums, and structural
/// presence** rather than raw addresses, user payloads, or per-identity
/// balances. No PII, IPFS hash content, dispute reasons, or beneficiary data
/// is embedded. Off-chain tooling must treat the raw snapshot and its XDR
/// as potentially-sensitive protocol state and protect it accordingly
/// (documented in `docs/upgrade-state-snapshot.md`).
#[contracttype]
#[derive(Clone, Eq, PartialEq)]
#[cfg_attr(any(test, feature = "testutils"), derive(Debug))]
pub struct UpgradeStateSnapshot {
    ...
    /// #1137: deterministic sum of `TotalLocked` across all funded tokens.
    pub total_locked: i128,
    /// #1137: deterministic sum of `TotalStaked` across all staking tokens.
    pub total_staked: i128,
    ...
    /// #1137: number of currently open recurring escrow records.
    pub recurring_escrow_count: u32,
    /// #1137: number of persisted batch escrow jobs awaiting execution.
...
impl CraftNexusContract {
    ...
    ///
    /// # Determinism (#1137)
    /// Every field is a deterministic projection of live storage: scalar
    /// counters, presence flags, next-ids, and lengths of bounded lists. No
    /// per-identity or user-supplied payload is embedded, so unchanged ledger
    /// state always yields an identical snapshot (and identical commitment).
    pub fn get_upgrade_state_snapshot(env: Env) -> UpgradeStateSnapshot {
        ...
        let has_pending_upgrade_proposal: bool = env
            ...
            .persistent()
            .has(&DataKey::WasmUpgradeProposal);
        // Batch jobs are stored under DataKey::BatchEscrowJob(id) with a
        // monotonic next-id under Symbol("next_batch_id"); the pending-job
        // surface is captured by the presence of any persisted job, which is
        // deterministic.
